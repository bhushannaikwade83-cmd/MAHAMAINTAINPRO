...
print(f"Fetching data for {len(seat_numbers)} selected students...\n")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data:
                ...
                status = "✓"
                print(f"{seat_no:<15} {student.get('student_name', 'N/A')[:40]:<40} {status:<10}")
            else:
                print(f"{seat_no:<15} {'Not found':<40} {'✗':<10}")
        else:
            print(f"{seat_no:<15} {'Error':<40} {'✗':<10}")
    except Exception as e:
        print(f"{seat_no:<15} {'Exception':<40} {'✗':<10}")
